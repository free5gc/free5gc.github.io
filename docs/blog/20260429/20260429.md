...
- **UERANSIM**: Up to ~100 concurrent UEs (`./nr-ue -c free5gc-ue.yaml -n 30`)
- **free-ran-ue**: Up to 3,000 simulated UEs
- **Custom webconsole scripts**: For multi-UE provisioning

![performance comparison with Open5GS](./image/performance%20comparison%20with%20Open5GS.png)


A notable benchmark: the safety-focused `select`-based `Submit` path incurs approximately **14% overhead** compared to a raw channel send (10.3s → 11.8s for 100 UE registrations via UERANSIM). This was deemed an acceptable trade-off for shutdown safety and deadlock prevention.
