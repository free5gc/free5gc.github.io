...
When people discuss UE registration performance in a 5G core network, the conversation usually focuses on the runtime signaling path: NAS messages, AMF state transitions, AUSF authentication, UDM subscription retrieval, and NRF service discovery. Those are important topics, but they are not the only bottlenecks that appear when preparing a large-scale free5GC testbed.

Before a UE can register successfully, the core network must already know who that UE is. In free5GC, this means the required subscriber data must exist in MongoDB: authentication material, access and mobility subscription data, session management subscription data, policy data, and optional GPSI-to-SUPI identity mapping. The free5GC WebConsole provides a convenient way to create this data. For a handful of UEs, the existing workflow is perfectly acceptable. For thousands of UEs, however, subscriber provisioning itself can become a visible preparation bottleneck.

This article describes an optimization in the free5GC WebConsole bulk subscriber creation path. The goal is not to accelerate the AMF/AUSF/UDM runtime registration procedure directly. Instead, the goal is to make **bulk subscriber provisioning** much faster, so experiments involving thousands of simulated UEs can be prepared quickly and repeatably.

The main improvement is a shift from a per-UE loop with many MongoDB operations to a design based on MongoDB-backed IMSI range allocation and collection-level bulk insertion. In practice, this reduces the time required to create thousands of subscriber records from minutes to less than a second for the tested 3,000-UE case, while preserving the existing WebConsole API shape.

## 2. Background: Subscriber Data in free5GC

A UE in a 5G system is identified by a SUPI, often represented as an IMSI in test environments. In free5GC WebConsole, the subscriber creation API receives a seed UE identifier such as `imsi-208930000100000`, a serving PLMN ID such as `20893`, and a subscriber profile payload. The WebConsole then stores the corresponding data into MongoDB collections consumed by free5GC network functions.

Although the user-facing operation looks like "create one subscriber", internally it touches multiple domains. Authentication data is needed by AUSF/UDM authentication flows. Access and mobility data contains information such as subscribed NSSAI and UE-AMBR. Session management data describes DNN and S-NSSAI configuration for PDU sessions. Policy data is used by policy-related components, and identity mapping can map GPSI values such as MSISDNs to SUPIs.
...
For example, if the request starts at `imsi-208930000100000` with `userNumber=3000`, the intended result is a continuous range from `imsi-208930000100000` to `imsi-208930000102999`, unless existing data requires the allocator to move the start forward.

The following table summarizes the major MongoDB collections involved in the provisioning path:

| Collection | Purpose |
|---|---|
| `subscriptionData.authenticationData.webAuthenticationSubscription` | WebConsole-side authentication subscription representation |
| `subscriptionData.authenticationData.authenticationSubscription` | Authentication subscription model used by the core |
| `subscriptionData.provisionedData.amData` | Access and mobility subscription data |
| `subscriptionData.provisionedData.smData` | Session management subscription data |
| `subscriptionData.provisionedData.smfSelectionSubscriptionData` | SMF selection subscription data |
| `policyData.ues.amData` | UE AM policy data |
| `policyData.ues.smData` | UE SM policy data |
| `policyData.ues.flowRule` | Optional flow rules |
| `policyData.ues.qosFlow` | Optional QoS flow definitions |
| `policyData.ues.chargingData` | Optional charging data |
| `subscriptionData.identityData` | Optional GPSI-to-SUPI identity mapping |
| `webui.idAllocator` | WebConsole allocator state for bulk IMSI reservation |

This multi-collection nature is the key reason the old implementation slowed down under large subscriber counts. Creating one UE is small. Creating thousands of UEs by repeating the one-UE workflow thousands of times creates a large number of database round trips.
