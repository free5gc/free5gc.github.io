# Integration free5GC and [OCUDU](https://ocudu.org/) With ZeroMQ
> [!NOTE]
> Author: [Fang-Kai Ting](https://github.com/qawl987)
> Date: 2026/06/19
---

This guide describes how to connect [**OCUDU**](https://ocudu.org/) to [**free5GC**](https://free5gc.org) and run a software UE with **srsUE** through **ZeroMQ**. It first covers a basic single-UE setup, then provides an advanced two-UE network slicing example with `iperf3` traffic testing.
...
2. Move the configuration files in the [appendix](../20260619/20260619.md#appendix) to the `~/ocudu-config` directory, and the multi-ue configuration files to the `~/ocudu-config/multi-ue` directory.
...
3. Add NAT for the UE subnet:
    ...
    The CIDR(10.60.0.0/16) is the IP class we set at webconsole for UE SNSSAI.
