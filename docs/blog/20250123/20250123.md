...
1. Load the 8021q kernel module using the “modprobe” command:

`sudo modprobe 8021q`

1. Verify that the module is loaded by running:

`lsmod | grep 8021q`

1. Create a VLAN subinterface for “enp0s8” with VLAN ID 10
...
1. create vxlan device, 'vxlan-demo' is the name of the interface, type should be vxlan.
...
1. Set vxlan-demo device interface on br19
...
1. Through tcpdump can see VXLAN encapsulates Layer 2 Ethernet frames in a Layer 3 UDP packet
...
1. Using `sudo ip netns exec h1 tcpdump -i any icmp` and `sudo tcpdump -i any icmp` can verify the following packet flow
