...
```
The sample yaml file. Only three key functions are shown here for illustrative purposes.
```yaml
# config.yaml
binaries:
    ...
    symbols:
      - name: "github.com/free5gc/ausf/internal/sbi/processor.(*Processor).UeAuthPostRequestProcedure"
        ...
        rets:
          ...
          - 0xb911b7
```
> Note: In a real production environment, this list can be expanded to hundreds of functions across all 5GC NFs (SMF, UPF, PCF, etc.).
